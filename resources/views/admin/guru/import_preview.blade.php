...
<div class="content-area">
    ...
    <div style="margin-bottom:24px;">
        <div style="display:grid; grid-template-columns:1fr 1fr; gap:16px;">
            <div style="background:#e0f2fe; border-left:4px solid #0891b2; padding:16px; border-radius:8px; display:flex; align-items:center; gap:12px;">
                <i class="ph ph-check-circle" style="color:#0369a1; font-size:28px;"></i>
                <div>
                    <div style="font-weight:700; color:#0369a1; font-size:14px;">{{ count($validRows) }} Data Valid</div>
                    <div style="font-size:12px; color:#0369a1; margin-top:2px;">Siap diimport ke database.</div>
                </div>
            </div>
    ...
    <div style="margin-bottom:24px; display:flex; justify-content:space-between; align-items:center;">
        <a href="{{ route('admin.guru.import-form') }}" class="btn btn-secondary">
            <i class="ph ph-arrow-left"></i> Unggah Ulang File
        </a>

        @if(count($validRows) > 0)
            <form method="POST" action="{{ route('admin.guru.import') }}">
                @csrf
                <input type="hidden" name="confirm" value="1">
                <button type="submit" class="btn btn-primary" style="padding:12px 24px; border-radius:10px; font-size:14.5px; background:#0891b2;">
    ...
    <div class="card" style="padding:0; overflow:hidden;">
        <div style="padding:20px 24px 12px; font-size:15px; font-weight:700; color:#1e293b; display:flex; align-items:center; gap:8px;">
            <i class="ph ph-check-circle" style="color:#0891b2; font-size:20px;"></i>
        ...
        <table style="width:100%; border-collapse:collapse;">
            <thead>
                <tr style="background:#f8fafc;">
                    ...
                    <th style="padding:10px 16px; text-align:left; font-size:11px; font-weight:600; color:#94a3b8; text-transform:uppercase; letter-spacing:0.5px; border-bottom:1px solid #f1f5f9;">NIK / NUPTK</th>
                    ...
                    <th style="padding:10px 16px; text-align:left; font-size:11px; font-weight:600; color:#94a3b8; text-transform:uppercase; letter-spacing:0.5px; border-bottom:1px solid #f1f5f9;">Status Kerja</th>
                ...
                @else
                    ...
                    <tr style="border-bottom:1px solid #f1f5f9;">
                        ...
                        <td style="padding:12px 16px; font-size:12.5px; color:#475569;">
                            <div>NIK: {{ $row['nik'] }}</div>
                            <div>NUPTK: {{ $row['nuptk'] ?? '—' }}</div>
                        </td>
                        <td style="padding:12px 16px; font-size:13px; color:#475569; font-weight:500;">
                            {{ $row['jabatan'] }}
                            @if($row['mata_pelajaran'])
                                <div style="font-size:11px; color:#94a3b8;">Mapel: {{ $row['mata_pelajaran'] }}</div>
                            @endif
                        </td>
                        <td style="padding:12px 16px; font-size:13px; color:#475569;">
                            {{ $row['jenis_kelamin'] }}
                        </td>
                        <td style="padding:12px 16px; font-size:12.5px; color:#475569; font-weight:600;">
                            {{ \App\Models\Guru::STATUS_KEPEGAWAIAN_LIST[$row['status_kepegawaian']] ?? $row['status_kepegawaian'] }}
                        </td>
                        <td style="padding:12px 20px; text-align:center;">
                            <span class="badge badge-{{ $row['status'] }}">{{ \App\Models\Guru::STATUS_LIST[$row['status']] ?? $row['status'] }}</span>
                        </td>
                    </tr>
                    @endforeach
                @endif
