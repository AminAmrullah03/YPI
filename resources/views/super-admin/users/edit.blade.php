...
@section('page-subtitle', 'Ubah Pengguna')
            ...
            <form method="POST" action="{{ route('super-admin.users.update', $user->id) }}">
                ...
                <div style="display:flex; flex-direction:column; gap:16px;">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="username" id="username" class="form-input @error('username') is-invalid @enderror" value="{{ old('username', $user->username) }}" required>
                    ...
                    <div class="form-group">
                        ...
                        <input type="password" name="password" id="password" class="form-input @error('password') is-invalid @enderror" placeholder="Kosongkan jika tidak ingin mengubah password...">
                        <small style="color:#94a3b8; font-size:11px; margin-top:4px; display:block;">Isi hanya jika ingin mengganti password lama.</small>
                        @error('password')
                            <span style="font-size:12px; color:#ef4444; margin-top:4px; display:block;">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="role" id="role" class="form-select @error('role') is-invalid @enderror" required onchange="toggleLembagaSelect()">
                            <option value="admin_lembaga" {{ old('role', $user->role) == 'admin_lembaga' ? 'selected' : '' }}>Admin Lembaga (Mengelola lembaga tertentu)</option>
                            <option value="super_admin" {{ old('role', $user->role) == 'super_admin' ? 'selected' : '' }}>Super Admin (Mengelola seluruh yayasan)</option>
                        </select>
                        @error('role')
                            <span style="font-size:12px; color:#ef4444; margin-top:4px; display:block;">{{ $message }}</span>
                        @enderror
                    ...
                    <div class="form-group" id="lembaga-group">
                        ...
                        <select name="lembaga_id" id="lembaga_id" class="form-select @error('lembaga_id') is-invalid @enderror">
                            ...
                            @foreach($lembagaList as $lem)
                                <option value="{{ $lem->id }}" {{ old('lembaga_id', $user->lembaga_id) == $lem->id ? 'selected' : '' }}>
